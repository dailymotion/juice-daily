...
function inlineDocument(document, css) {
  var rules = utils.parseCSS(css)
    , editedElements = []
    , reg = new RegExp('#[0-9a-fA-F]+', 'g')

  rules.forEach(handleRule);
  editedElements.forEach(setStyleAttrs);

  // convert 3 digit hex color to 6 digit hex color
  function convertTo6Hex(color) {
    var color = color.replace('#', '').match(/.{1,1}/g);
    return '#' + color.map(function(n){ return n + n; }).join('');
  }
    ...
    utils.toArray(els).forEach(function (el) {
      ...
      function addProps (style, selector) {
        for (var i = 0, l = style.length; i < l; i++) {
          var name = style[i]
            , value = style[name]
            , hasColor = value.match(reg)

          if (hasColor && hasColor[0].length === 4) {
            value = convertTo6Hex(hasColor[0]);
          }
      ...
      function addAttrs(style, selector) {
        for (var i = 0, l = style.length; i < l; i++) {
          var name = style[i]
            , value = style[name]
            , hasColor = value.match(reg)
          ...
          if (typeof htmlStyles[name] !== 'undefined')
          {
            if (name == 'border')
            {
                var border = value.split(' ');
                value = border.length == 1 ? border[0] : false;
            }

            el.getAttribute(name) || (value && el.setAttribute(htmlStyles[name], value));
          }
        }
      }

      addProps(style, selector);
      addAttrs(style, selector);
    });
  }
